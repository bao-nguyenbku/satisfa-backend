...
export class SatisgiService {
  ...
  async getRandomMessage(): Promise<any> {
    const randomIdx = Math.floor(Math.random() * this.sentences.length);
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve(this.sentences[randomIdx]);
      }, 3000);
    });
  }
}
